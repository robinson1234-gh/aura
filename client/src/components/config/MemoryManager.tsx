...
import {
  ...
  AlertCircle, CheckCircle2, Clock, Edit2, Hash, MessageSquare,
} from 'lucide-react';
...
export function MemoryManager({ workspacePath, activeSessionId, sessions = [] }: Props) {
  const [memories, setMemories] = useState<MemoryEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<Category | 'all'>('all');
  const [selectedSessionId, setSelectedSessionId] = useState<string | undefined>(activeSessionId);
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (selectedSessionId) {
        const data = await api.memory.listBySession(selectedSessionId);
        setMemories(data);
      } else {
        const data = await api.memory.list(workspacePath, true);
        setMemories(data);
      }
    } catch { setMemories([]); }
    setLoading(false);
  }, [workspacePath, selectedSessionId]);

  useEffect(() => { load(); }, [load]);

  const handleAdd = async () => {
    if (!newContent.trim()) return;
    await api.memory.create(workspacePath, newCategory, newContent.trim(), selectedSessionId);
    setNewContent('');
    setAdding(false);
    load();
  };
  ...
  const handleShowContext = async () => {
    ...
    try {
      const data = await api.memory.getContext(workspacePath, selectedSessionId);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Memories are scoped per chat session. Each conversation builds its own context that the agent remembers.
        </p>
      </div>
      ...
      <div className="mb-4 p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">Chat Session</span>
        </div>
        <select
          value={selectedSessionId || ''}
          onChange={e => setSelectedSessionId(e.target.value || undefined)}
          className="w-full text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
        >
        ...
        {selectedSession && (
          <p className="mt-1.5 text-[10px] text-slate-400 font-mono">
            Session: {selectedSession.id.slice(0, 8)}... | Workspace: {workspacePath}
          </p>
        )}
        {!selectedSessionId && (
          <p className="mt-1.5 text-xs text-amber-600 dark:text-amber-400">
            Select a chat session to view and manage its memories.
          </p>
        )}
      </div>
      ...
      {!selectedSessionId ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-400">Select a chat session above to manage its memories</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin space-y-2">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-400">No memories for this chat yet</p>
                <p className="text-xs text-slate-400 mt-1">Memories will be automatically extracted from this conversation, or you can add them manually.</p>
              </div>
            ) : (
              filtered.map(m => {
          ...
          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 flex items-start gap-2">
            ...
            <p className="text-[10px] text-slate-400 leading-relaxed">
              <strong>Session-scoped memory</strong>: Each chat has its own memories, automatically extracted from the conversation.
              <strong> Three types</strong>: Semantic (knowledge), Episodic (experiences), Procedural (preferences).
              <strong> Conversation summaries</strong>: Long sessions are automatically summarized so older context is not lost.
              All memories are injected into the LLM system prompt for that chat.
            </p>
          </div>
        </>
      )}
